...
class Plugin {
    ...
    function check_settings() {
        ...
        if ( isset( $_POST ) && ! empty( $_POST ) && isset( $_GET['page'] ) && strpos( $_GET['page'], 'security-safe' ) !== false ) {
            ...
            } elseif ( $_GET['page'] == 'security-safe-files' && isset( $_GET['tab'] ) && in_array( $_GET['tab'], array( 'core', 'theme', 'plugins', 'uploads' ) ) ) {

                // File Permissions Processed
                $this->messages[] = array( 'Your request has been processed. Please refer to the log below for details.', 1, 1 );
